...
from src.validators.validators import validator_memberATE, validator_change_status_member, validator_find_accounts, \
    validator_id, validator_memberATE_change, validator_memberATE_password
...
@account.route("/accounts/<filterSearch>/<criterion>", methods=["GET"])
@Auth.requires_token
@Auth.requires_role(AccountRole.MANAGER.name)
def find_accounts(filterSearch, criterion):
    json_validator = {"filterSearch": filterSearch, "criterion": criterion}
    response = Response(json.dumps(json_error(ResponsesREST.INVALID_INPUT.value)),
                        status=ResponsesREST.INVALID_INPUT.value, mimetype="application/json")
    if validator_find_accounts.is_valid(json_validator):
        get_accounts = Account()
        result = get_accounts.consult_list_accounts(filterSearch, criterion)
...
def change_account(accountId):
    ...
    if all(key in json_values for key in values_required):
        json_validator = json_values
        json_validator["idAccount"] = int(accountId)
        if validator_memberATE_change.is_valid(json_validator):
            account_change = Account()
            account_change.id_memberATE = int(accountId)
...
@account.route("/accounts/password/<accountId>", methods=["PATCH"])
@Auth.requires_token
