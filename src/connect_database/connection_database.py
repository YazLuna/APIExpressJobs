"""This module manages the connection to the database and the queries."""
...
class Connection:
    """This class manages the connection to the database and the queries."""
    static_host = ""
    static_database = ""
    static_user = ""
    static_password = ""
    ...
    def build_from_static():
        """This function generates everything necessary to connect to the database."""
        connection = None
    ...
    def select(self, query, values: list = None):
        """This function executes a search query to the database."""
        results = []
        if self.host is not None:
            parameters: tuple = ()
            try:
